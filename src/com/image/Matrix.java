...
public class Matrix {
     ...
     * @param col - 元素的列
     */
    public void setRC(int row, int col, double value)
    {
       elements[col + row * numColumns] = value;
    }
    
    public double getRC(int row, int col)
    {
    ...
    {
        numRows = value[0].length;
        numColumns = value[1].length;
     ...
     * @throws Exception 
     */
    public static Matrix operatorAdd(Matrix m1, Matrix m2) throws Exception
    {
        return m1.Add(m2);
    }
     ...
     * @throws Exception 
     */
    public static Matrix operatorDe(Matrix m1, Matrix m2) throws Exception
    {
     ...
     * @throws Exception 
     */
    public static Matrix operatorMu(Matrix m1, Matrix m2) throws Exception
    {
     ...
     * @return double[]对象
     */
    public static double[] operatorArr (Matrix m)
    {
        return m.elements;
    }
    ...
    public String toString() {
        return "Matrix [numColumns=" + numColumns + ", numRows=" + numRows + ", eps=" + eps + ", elements="
     ...
     * 
     * @param value
     *            - 一维数组，长度为numColumns*numRows，存储 矩阵各元素的值
     */
    ...
    public boolean Multiply(Matrix AR, Matrix AI, Matrix BR, Matrix BI, Matrix CR, Matrix CI) {
        // 首先检查行列数是否符合要求
        if (AR.GetNumColumns() != AI.GetNumColumns() || AR.GetNumRows() != AI.GetNumRows()
                || BR.GetNumColumns() != BI.GetNumColumns() || BR.GetNumRows() != BI.GetNumRows()
     ...
     * @return bool型，求解是否成功
     */
    public boolean ComputeDetCholesky(double realDetValue)
    { 
        int i,j,k,u,l;
        double d;
     ...
     * @param dblC
     *            - 一维数组，长度为矩阵的阶数，前n-1个元素返回对称三对角阵的 次对角线元素
     * @return bool型，求解是否成功
     */
     ...
     * @param dblC
     *            - 一维数组，长度为矩阵的阶数，前n-1个元素传入对称三对角阵的 次对角线元素
     * @param mtxQ
     *            - 如果传入单位矩阵，则返回实对称三对角阵的特征值向量矩阵；
     *            如果传入MakeSymTri函数求得的矩阵A的豪斯荷尔德变换的乘积
     *            矩阵Q，则返回矩阵A的特征值向量矩阵。其中第i列为与数组dblB 中第j个特征值对应的特征向量。
     * @param nMaxIt
     *            - 迭代次数
     * @param eps
     *            - 计算精度
     ...
     * 
     * @param dblU
     *            - 一维数组，长度为矩阵的阶数，返回时存放特征值的实部
     * @param dblV
     *            - 一维数组，长度为矩阵的阶数，返回时存放特征值的虚部
     * @param nMaxIt
     *            - 迭代次数
     * @param eps
     *            - 计算精度
    ...
    public boolean ComputeEvHBerg(double[] dblU, double[] dblV, int nMaxIt, double eps) {
        ...
        while (m != 0) {
            l = m - 1;
            while ((l > 0) && (Math.abs(elements[l * n + l - 1]) > eps
                    * (Math.abs(elements[(l - 1) * n + l - 1]) + Math.abs(elements[l * n + l]))))
                l = l - 1;
     ...
     * 
     * @param dblEigenValue
     *            - 一维数组，长度为矩阵的阶数，返回时存放特征值
     * @param mtxEigenVector
     *            - 返回时存放特征向量矩阵，其中第i列为与数组 dblEigenValue中第j个特征值对应的特征向量
     * @param nMaxIt
     *            - 迭代次数
     * @param eps
     *            - 计算精度
